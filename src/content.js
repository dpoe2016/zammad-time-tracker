  ...
  window.ZammadTimetracker = class {
      ...
      try {
        ...
        if (apiSettings.baseUrl && apiSettings.token) {
          ...
          if (window.zammadApi) {
            ...
            console.log(
              'Global Zammad API not available, creating local instance'
            );
            try {
              ...
              console.error(
                'Error creating local Zammad API instance:',
                apiError
              );
            }
          }
    ...
    async getCurrentTicketId() {
      ...
      if (
        ...
        try {
          console.log(
            'Trying to get ticket ID from API using URL-extracted ID as reference:',
            urlTicketId
          );
          ...
          console.log(
            'Could not get ticket ID from API, falling back to DOM extraction'
          );
        } catch (error) {
    ...
    extractTicketIdFromDom() {
      ...
      const selectors = [
        ...
        'h1',
        'h2',
        'h3', // Headers that often contain ticket info
      ];
    ...
    setupMessageListener() {
      ...
      chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        ...
        switch (request.action) {
          ...
          case 'showAnswerDialog':
            // Handle showing answer dialog
            (async () => {
              try {
                const currentTicketId = await this.getCurrentTicketId();
                if (!currentTicketId) {
                ...
                this.showAnswerDialog(currentTicketId);
                sendResponse({ success: true, ticketId: currentTicketId });
              } catch (error) {
                console.error('Error showing answer dialog:', error);
                sendResponse({ success: false, error: error.message });
              }
            })();
            return true; // Indicate we'll send response asynchronously
              ...
              try {
                ...
                if (
                  ...
                  try {
                    ...
                    const ticketData =
                      await window.zammadApi.getTicket(currentTicketId);
                    ...
                    if (ticketData) {
                      ...
                      try {
                        const timeEntries =
                          await window.zammadApi.getTimeEntries(
                            currentTicketId
                          );
                        console.log(
                          'Time entries received from API:',
                          timeEntries
                        );
                        ...
                        console.error(
                          'Error getting time entries from API:',
                          timeError
                        );
                    ...
                    console.error(
                      'Error getting ticket info from API:',
                      apiError
                    );
                ...
                try {
                  const titleElement = document.querySelector(
                    '.ticket-title, .ticketZoom-header .ticket-number + div, h1, h2'
                  );
                  if (titleElement) {
                ...
                try {
                  const timeField = document.querySelector(
                    'input[name="time_unit"], input[id*="time"], .time-accounting input'
                  );
    ...
    async startTracking() {
      try {
        ...
        console.log(
          `Time tracking started for ticket #${this.ticketId}${ticketTitle ? ` (${ticketTitle})` : ''}`
        );
        return true;
      } catch (error) {
    ...
    async stopTracking(comment = '') {
      ...
      try {
        ...
        console.log(
          `Time tracking ended for ticket #${this.ticketId}. Duration: ${formatDuration(duration)}`
        );
        return true;
      } catch (error) {
    ...
    async submitTimeEntry(durationInSeconds, userComment = '') {
      try {
        ...
        if (
          ...
          throw new Error(
            'API not configured. Please configure API settings in the extension options.'
          );
        }
      } catch (error) {
    ...
    async loadTrackingState() {
      try {
        ...
        if (state && state.isTracking && state.startTime) {
          ...
          if (state.title) {
            console.log(
              `Time tracking continued for ticket #${this.ticketId} (${state.title})`
            );
          } else {
            ...
            if (
              ...
              try {
                console.log(
                  'Getting ticket info from API for restored tracking'
                );
                const ticketData = await window.zammadApi.getTicket(
                  this.ticketId
                );
                if (ticketData && ticketData.title) {
                  ...
                  console.log(
                    'Updated tracking state with title from API:',
                    ticketData.title
                  );
                }
              } catch (error) {
                console.error(
                  'Error getting ticket info from API during state restore:',
                  error
                );
                // Continue without ticket title
              }
          ...
          if (currentTicketId && currentTicketId !== state.ticketId) {
            console.log(
              `Note: You're viewing ticket #${currentTicketId} but tracking is active for ticket #${state.ticketId}`
            );
        ...
        try {
          ...
          if (
            ...
            chrome.runtime.sendMessage({
              action: 'showNotification',
              title: 'Success',
              message: `Answer sent to ticket #${ticketId}`,
            });
          ...
          chrome.runtime.sendMessage({
            action: 'showNotification',
            title: 'Error',
            message: `Failed to send answer: ${error.message}`,
          });
...
if (window.zammadTrackerInstance === undefined) {
  ...
  new MutationObserver(() => {
    ...
    if (url !== lastUrl) {
      ...
      setTimeout(() => {
        if (
          !window.zammadTrackerInstance ||
          window.zammadTrackerInstance.needsReinit
        ) {
