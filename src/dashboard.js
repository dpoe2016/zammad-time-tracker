...
class ZammadDashboard {
  constructor() {
    ...
    try {
      ...
      this.editTimeTrackingItem = document.getElementById(
      ...
      this.organizationFilterLabel = document.getElementById(
      ...
      this.columnVisibilityText = document.getElementById(
        'columnVisibilityText'
      );

      // Ticket info sidebar elements
      this.ticketInfoSidebar = document.getElementById('ticketInfoSidebar');
      ...
      this.editTimeTrackingText = document.getElementById(
  ...
  async initPinnedTickets() {
    try {
      ...
      logger.info(
        `Loaded ${this.pinnedTickets.size} pinned tickets from storage`
      );
    } catch (error) {
  ...
  updateUILanguage() {
    ...
    this.organizationFilterLabel.textContent =
      t('dashboard_org_filter') || 'Organization:';
    ...
    try {
      chrome.storage.onChanged.addListener((changes, area) => {
        if (area === 'local' && changes.zammadApiSettings) {
          logger.info(
            'Detected change in API settings, re-evaluating auto-refresh'
          );
          this.applyAutoRefreshFromSettings();
        }
      });
      ...
      logger.warn(
        'chrome.storage.onChanged not available or failed to attach listener',
        e
      );
    }
  ...
  async applyAutoRefreshFromSettings() {
    try {
      ...
      if (nextSec !== this.autoRefreshSec) {
        ...
        logger.info(
          `Auto-refresh interval set to ${this.autoRefreshSec} seconds`
        );
  ...
  ensureTicketDragListeners() {
    ...
    logger.info(
      `Ensuring drag listeners for ${allTicketItems.length} ticket items`
    );
  ...
  setupDropZone(container, category) {
    ...
    container.addEventListener('drop', (event) => {
      ...
      const currentCategory = this.draggedTicket.getAttribute(
      ...
      if (currentCategory === category) {
        logger.info(
          `Ticket #${ticketId} dropped in the same category (${category})`
        );
        return;
      }
  ...
  setupAgentDropZone(container, agentId, agentName) {
    ...
    container.addEventListener('drop', (event) => {
      ...
      const currentAgentId =
        currentOwnerId === 'null' || !currentOwnerId ? null : currentOwnerId;

      // If dropped on the same agent, do nothing
      if (currentAgentId == targetAgentId) {
        logger.info(
          `Ticket #${ticketId} dropped on the same agent (${agentName})`
        );
        return;
      }

      logger.info(
        `Ticket #${ticketId} dropped on agent: ${agentName} (${agentId})`
      );
  ...
  setupGroupDropZone(container, groupId, groupName) {
    ...
    container.addEventListener('drop', (event) => {
      ...
      const currentTicketGroupId =
        currentGroupId === 'none' || !currentGroupId ? null : currentGroupId;

      // If dropped on the same group, do nothing
      if (currentTicketGroupId == targetGroupId) {
        logger.info(
          `Ticket #${ticketId} dropped on the same group (${groupName})`
        );
        return;
      }

      logger.info(
        `Ticket #${ticketId} dropped on group: ${groupName} (${groupId})`
      );
  ...
  async updateTicketGroup(ticketId, newGroupId, groupName) {
    try {
      logger.info(
        `Updating ticket #${ticketId} group to: ${groupName} (${newGroupId || 'no group'})`
      );
      ...
      logger.info(
        `Successfully updated ticket #${ticketId} group to ${groupName}`
      );
  ...
  async updateTicketState(ticketId, category) {
    try {
      ...
      logger.info(
        `Successfully updated ticket #${ticketId} to ${stateName} state`
      );
  ...
  async updateTicketOwner(ticketId, newOwnerId, agentName) {
    try {
      logger.info(
        `Updating ticket #${ticketId} owner to: ${agentName} (${newOwnerId || 'unassigned'})`
      );
      ...
      logger.info(
        `Successfully updated ticket #${ticketId} owner to ${agentName}`
      );
  ...
  async initializeApi() {
    try {
      ...
      if (!apiSettings || !apiSettings.baseUrl || !apiSettings.token) {
        this.showError(
          'API settings not configured. Please configure in the extension options.'
        );
        return;
      }
  ...
  async loadTickets(forceRefresh = false) {
    ...
    try {
      logger.info(
        `Loading tickets from API${forceRefresh ? ' (forcing refresh)' : ''}`
      );
      ...
      try {
        ...
        logger.warn(
          'Failed to load groups, continuing without group information:',
          error
        );
        this.groups = [];
      }
      ...
      try {
        ...
        logger.info(
          `Loaded ${this.organizations.length} organizations from API`
        );
      } catch (error) {
        logger.warn(
          'Failed to load organizations, continuing without organization information:',
          error
        );
        this.organizations = [];
      }
  ...
  async fetchFirstArticleForTickets() {
    ...
    logger.info(
      `Fetching first article content for ${this.tickets.length} tickets`
    );
        ...
        try {
          ...
          if (articles && articles.length > 0) {
            ...
            ticket.first_article_content =
              firstArticle.body || firstArticle.content || '';
            logger.debug(`Added first article content to ticket ${ticket.id}`);
          }
        } catch (error) {
          logger.warn(
            `Failed to fetch articles for ticket ${ticket.id}:`,
            error
          );
  ...
  setupStateView() {
    ...
    const stateColumns = [
      {
        id: 'newTickets',
        titleId: 'newColumnTitle',
        countId: 'newCount',
        title: 'New',
      },
      {
        id: 'openTickets',
        titleId: 'openColumnTitle',
        countId: 'openCount',
        title: 'Open',
      },
      {
        id: 'progressTickets',
        titleId: 'progressColumnTitle',
        countId: 'progressCount',
        title: 'In Progress',
      },
      {
        id: 'waitingTickets',
        titleId: 'waitingColumnTitle',
        countId: 'waitingCount',
        title: 'Waiting',
      },
      {
        id: 'closedTickets',
        titleId: 'closedColumnTitle',
        countId: 'closedCount',
        title: 'Closed',
      },
    ];
  ...
  getAgentName(ticket) {
    ...
    if (
      ticket.owner_data &&
      ticket.owner_data.firstname &&
      ticket.owner_data.lastname
    ) {
  ...
  processTickets() {
    ...
    logger.info(
      `Filtered to ${filteredTickets.length} tickets from ${this.tickets.length} total`
    );
  ...
  processTicketsByState(sortedTickets) {
    ...
    logger.info(
      `Displayed tickets: ${newCount} new, ${openCount} open, ${progressCount} in progress, ${waitingCount} waiting, ${closedCount} closed`
    );
  }
  ...
  async populateGroupFilterFromTickets() {
    ...
    const groupsById = new Map(
      (this.groups || []).map((g) => [String(g.id), g])
    );
    const options = [];
    ...
    const toRemove = Array.from(this.groupFilter.options).filter(
      (opt) => opt.value !== 'all'
    );
    ...
    if (
      currentSelection &&
      Array.from(this.groupFilter.options).some(
  ...
  applyGroupFilter() {
    ...
    const selectedOrg = this.organizationFilter
    ...
    lists.forEach((list) => {
      ...
      items.forEach((item) => {
        ...
        const groupMatch =
          selectedGroup === 'all' ? true : itemGroupId === selectedGroup;
        const orgMatch =
          selectedOrg === 'all' ? true : itemOrgId === selectedOrg;
  ...
  applyOrganizationFilter() {
    ...
    const selectedGroup = this.groupFilter
      ? this.groupFilter.value || 'all'
      : 'all';
  ...
  updateCountsFromDOM() {
    const countVisible = (list) =>
      list
        ? Array.from(list.querySelectorAll('.ticket-item')).filter(
  ...
  createTicketElement(ticket) {
    ...
    if (ticket.organization_id) {
      ticketItem.setAttribute(
        'data-organization-id',
        String(ticket.organization_id)
      );
    } else {
...
<!--            <span class="ticket-item-organization">${organizationName ? `${organizationName}` : 'No Organization'}</span>-->
    ...
    setTimeout(() => {
      ticketItem.addEventListener('click', (event) => {
        ...
        if (
          event.target.matches('.ticket-pin-button') ||
        ...
        if (event.ctrlKey || event.metaKey) {
          ...
          // Normal click - open sidebar with ticket details
          event.preventDefault();
          event.stopPropagation();
          this.openSidebar(ticket, ticketItem);
        }
      });
    }, 10);
    ...
    ticketItem.addEventListener('contextmenu', (event) => {
      ...
      const ticket = this.tickets.find(
        (t) => String(t.id) === String(ticketId) || t.id == ticketId
      );
   ...
   */
  getCurrentUserId() {
    return zammadApi && zammadApi.currentUserId
      ? zammadApi.currentUserId
      : null;
  }
  ...
  isTicketCreatedToday(ticket) {
    ...
    try {
      ...
      logger.warn(
        `Failed to parse created_at date for ticket ${ticket.id}:`,
        error
      );
      return false;
    }
  ...
  isTicketOld(ticket) {
    ...
    try {
      ...
      logger.warn(
        `Failed to parse updated_at date for ticket ${ticket.id}:`,
        error
      );
      return false;
    }
  ...
  async populateOrganizationFilterFromTickets() {
    ...
    const organizationsById = new Map(
      (this.organizations || []).map((o) => [String(o.id), o])
    );
    const options = [];
    ...
    // Clear existing dynamic options (keep 'all')
    const toRemove = Array.from(this.organizationFilter.options).filter(
      (opt) => opt.value !== 'all'
    );
    toRemove.forEach((opt) => opt.remove());
    ...
    if (
      currentSelection &&
      Array.from(this.organizationFilter.options).some(
        (opt) => opt.value === currentSelection
      )
    ) {
  ...
  showColumnVisibilityMenu() {
    ...
    setTimeout(() => {
      document.addEventListener(
        'click',
        this.closeColumnMenuOnClickOutside.bind(this)
      );
    }, 100);
  }
  ...
  closeColumnMenuOnClickOutside(event) {
    ...
    if (
      ...
      !this.columnVisibilityBtn.contains(event.target)
    ) {
  ...
  async loadTrackingState() {
    try {
      ...
      if (state && state.isTracking) {
        ...
        logger.info('Restored time tracking session', {
  ...
  async startTimeTracking() {
    try {
      ...
      if (!ticketMatch) {
        ...
        this.showMessage(
          'Please navigate to a Zammad ticket page to start tracking',
          'warning'
        );
        return;
      }
  ...
  async handleTimeSubmission() {
    ...
    try {
      ...
      if (zammadApi && zammadApi.isInitialized()) {
        ...
        await zammadApi.submitTimeEntry(
  ...
  startTimer() {
    ...
    this.timerInterval = setInterval(() => {
      ...
      if (this.isTracking && Date.now() % 30000 < 1000) {
  ...
  initContextMenu() {
    ...
    if (this.addCommentItem) {
      console.log('Adding event listener to addCommentItem');
      this.addCommentItem.addEventListener('click', (e) => {
        console.log('Add comment item clicked');
        e.preventDefault();
        e.stopPropagation(); // Prevent click from closing menu
        logger.info('Add comment context menu item clicked');
  ...
  async startTimeTrackingForTicket() {
    ...
    try {
      ...
      const ticketId =
        this.currentTicketData.id || this.currentTicketData.number;
      const ticketTitle = this.currentTicketData.title || `Ticket #${ticketId}`;

      if (!ticketId) {
        logger.error(
          'No ticket ID found in current ticket data',
          this.currentTicketData
        );
        return;
      }
  ...
  editTimeTracking() {
    ...
    if (!ticketData) {
      console.error(
        'No current ticket data available for editing time tracking'
      );
      logger.error(
        'No current ticket data available for editing time tracking'
      );
  ...
  /**
   * Add comment to ticket from context menu
   */
  async addCommentToTicket() {
    console.log('addCommentToTicket called');
    ...
    if (!ticketData) {
      console.error('No current ticket data available for adding comment');
      logger.error('No current ticket data available for adding comment');
      alert(
        'No ticket data available. Please try right-clicking on the ticket again.'
      );
      return;
    ...
    try {
      ...
      console.log('Opening comment dialog for ticket', ticketId);
      logger.info('Opening comment dialog for ticket', { ticketId });

      // Show comment dialog
      this.showCommentDialog(ticketId);
    } catch (error) {
      console.error('Failed to add comment to ticket:', error);
      logger.error('Failed to add comment to ticket:', error);
      alert('Error opening comment dialog: ' + error.message);
    }
  }
  ...
  showCommentDialog(ticketId) {
    console.log('showCommentDialog called for ticket:', ticketId);
    ...
    submitButton.onclick = async () => {
      ...
      try {
        // Submit comment via API (use global zammadApi object)
        ...
        if (typeof zammadApi !== 'undefined' && zammadApi.isInitialized()) {
          console.log('Submitting comment via API...');
          ...
          logger.info('Comment created successfully for ticket', { ticketId });

          alert(`Comment added to ticket #${ticketId}`);
          overlay.remove();

          // Refresh the tickets to show the updated comment count
          self.loadTickets();
        } else {
    ...
    try {
      ...
      entriesList.innerHTML = timeEntries
        .map((entry, index) => {
          ...
          const activity =
            entry.comment || entry.activity_type || 'No description';
    ...
    try {
      ...
      if (zammadApi && zammadApi.isInitialized && zammadApi.isInitialized()) {
        ...
        this.showToastNotification(
          t('manual_entry_required', [minutes]),
          'warning'
        );
      }
    } catch (error) {
  ...
  editTimeEntry(entry, ticketId, modalContent) {
    ...
    const entryElement = modalContent.querySelector(
    ...
    entryElement
      ...
      .addEventListener('click', async () => {
        const newTime = parseInt(
          entryElement.querySelector(`#editTimeInput-${entry.id}`).value
        );
        const newActivity = entryElement
          .querySelector(`#editActivityInput-${entry.id}`)
          .value.trim();
        ...
        await this.saveEditedTimeEntry(
          entry,
          ticketId,
          newTime,
          newActivity,
    ...
    entryElement
      ...
      .addEventListener('click', () => {
        ...
        this.attachEntryButtonListeners(
          entryElement,
          entry,
          ticketId,
          modalContent
  ...
  async saveEditedTimeEntry(
    ...
    try {
      ...
      if (zammadApi && zammadApi.isInitialized && zammadApi.isInitialized()) {
        ...
        logger.warn(
          'API not available, cannot update time entry automatically'
        );
        this.showToastNotification(
          'API not available for updating entries',
          'warning'
        );
      }
    } catch (error) {
  ...
  async deleteTimeEntry(entry, ticketId, modalContent) {
    const confirmDelete = confirm(
      `Are you sure you want to delete this ${entry.time_unit || 0} minute time entry?`
    );
    ...
    try {
      ...
      if (zammadApi && zammadApi.isInitialized && zammadApi.isInitialized()) {
        ...
        logger.warn(
          'API not available, cannot delete time entry automatically'
        );
        this.showToastNotification(
          'API not available for deleting entries',
          'warning'
        );
      }
    } catch (error) {
  ...
  async highlightTrackingTicket(ticketId) {
    ...
    try {
      logger.info(
        'Attempting to highlight ticket and check for other active tracking:',
        ticketId,
        typeof ticketId
      );
  ...
  clearTicketHighlights() {
    try {
      const highlightedTickets = document.querySelectorAll(
        '.time-tracking-active, .time-tracking-other'
      );
      highlightedTickets.forEach((ticket) => {
        ticket.classList.remove('time-tracking-active', 'time-tracking-other');
      });
      logger.info(
        'Cleared all ticket highlights, count:',
  ...
  async highlightAllActiveTrackingTickets() {
    try {
      ...
      if (this.isTracking && this.currentTicketId) {
        const currentTicketElement = document.querySelector(
          `[data-ticket-id="${String(this.currentTicketId)}"]`
        );
        if (currentTicketElement) {
          currentTicketElement.classList.add('time-tracking-active');
          logger.info(
            'Highlighted current dashboard tracking ticket (blue):',
            this.currentTicketId
          );
        }
      }
        ...
        if (globalTicketId !== currentTicketId || !this.isTracking) {
          ...
          if (ticketElement) {
            ...
            if (globalTicketId !== currentTicketId) {
              ...
              logger.info(
                'Highlighted other instance tracking ticket (orange):',
                globalTicketId
              );
            }
          }
  ...
  testHighlightFirstTicket() {
    try {
      ...
      if (allTickets.length > 0) {
        ...
        logger.info(
          'Ticket classes after adding highlight:',
          firstTicket.className
        );
  ...
  /**
   * Open sidebar with ticket information
   * @param {Object} ticket - The ticket data object
   * @param {HTMLElement} ticketElement - The ticket element being clicked
   */
  async openSidebar(ticket, ticketElement) {
    logger.info('Opening sidebar for ticket:', ticket.id);
    
    if (!this.ticketInfoSidebar || !this.sidebarContent) {
      logger.warn('Sidebar elements not found');
      return;
    }

    // Show loading state
    this.sidebarContent.innerHTML = '<p class="sidebar-placeholder">Loading ticket details...</p>';
    this.ticketInfoSidebar.classList.add('open');

    // Reuse the tooltip content generation logic
    try {
  ...
  closeSidebar() {
    ...
    if (this.ticketInfoSidebar) {
      this.ticketInfoSidebar.classList.remove('open');
    }
  }

  /**
   * Generate ticket detail content for sidebar (reuses tooltip logic)
   * @param {Object} ticket - The ticket data object
   * @returns {string} HTML content for the sidebar
   */
  async generateTicketDetailContent(ticket) {
    // Get ticket details
    ...
    if (!ticket.enhanced_content_cache && ticket.id) {
      try {
        ...
        if (articles && articles.length > 0) {
          ...
          let processedFirstContent = firstContent
            .replace(/<[^>]*>/g, '')
            .replace(/&[^;]+;/g, '')
            .trim();
          if (processedFirstContent.length > 400) {
            processedFirstContent =
              processedFirstContent.substring(0, 400) + '...';
          }
          ...
          const customerArticles = articles.filter(
            (article) =>
              article.type_id === 1 ||
              (article.from && article.from.includes('@'))
          );
