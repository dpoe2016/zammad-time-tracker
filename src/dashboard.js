...
<!--            <span class="ticket-item-organization">${organizationName ? `${organizationName}` : 'No Organization'}</span>-->
  ...
  async showTicketTooltip(ticketElement, ticket) {
      ...
      if (this.userCache && this.userCache.has(ticket.owner_id)) {
        ...
        if (user) {
          ...
          try {
            ...
            logger.warn(
              `Failed to fetch user info for ${ticket.owner_id}:`,
              error
            );
            userName = `User ${ticket.owner_id}`;
          }
    ...
    let customerEmail = '';
    console.log(
      `Tooltip for ticket ${ticketId}: customer_id=${ticket.customer_id}, customer_data=`,
    ...
    if (ticket.customer_id || ticket.customer) {
      if (ticket.customer_data) {
        ...
        console.log(
          `Using customer_data: name=${customerName}, email=${customerEmail}`
        );
      } else if (ticket.customer && typeof ticket.customer === 'object') {
            ...
            <div class="ticket-tooltip-body">
                ...
                ${
                  firstArticleContent
                    ...
                    ${
                      lastArticleContent &&
                      lastArticleContent !== firstArticleContent
                        ? `<div class="ticket-tooltip-field content-field">
  ...
  positionTooltip(tooltip, ticketElement) {
    ...
    if (arrowPosition === 'left' || arrowPosition === 'right') {
      ...
      arrowTop = Math.max(
        ...
        Math.min(
          85,
          ((ticketCenter - tooltipTopPosition) / tooltipHeight) * 100
        )
      );
    }
    ...
    tooltip.className = tooltip.className.replace(
      /arrow-(top|bottom|left|right)/g,
      ''
    );
    tooltip.classList.add(`arrow-${arrowPosition}`);

    logger.debug(
      `Positioned tooltip at (${absoluteLeft}, ${absoluteTop}) with arrow ${arrowPosition} at ${arrowTop}%`
    );
  }
