...
function updateUILanguage() {
  ...
  document.getElementById('statusText').textContent = t(
    document.querySelector('.status-dot').classList.contains('active')
      ? 'status_active'
      : 'status_inactive'
  );
  ...
  document.getElementById('notificationsLabel').textContent =
    t('notifications');
...
class TimetrackingPopup {
  ...
  initCacheRefreshListeners() {
    ...
    window.addEventListener('ticketCacheRefreshed', (event) => {
      ...
      this.debug(
        `Cache refreshed for key: ${cacheKey}, tickets: ${tickets.length}`
      );
      ...
      if (
        ...
        if (shouldUpdate) {
          ...
          this.ticketsInfo.textContent =
            t('tickets_loaded', [nonClosedCount]) +
            ' (' +
            t('non_closed_only') +
            ') - ' +
            t('refreshed');
  ...
  async populateUserFilter() {
    ...
    try {
      ...
      if (settings.userIds) {
        configuredUserIds = settings.userIds
          .split(',')
          .map((id) => id.trim())
          .filter((id) => id);
        this.debug(
          `Found configured user IDs: ${configuredUserIds.join(', ')}`
        );
      }
    } catch (error) {
    ...
    const filteredUsers =
      configuredUserIds.length > 0
        ? sortedUsers.filter((user) =>
            configuredUserIds.includes(String(user.id))
          )
        : sortedUsers;
  ...
  async loadAssignedTickets() {
    ...
    try {
      ...
      if (this.selectedUserId === 'all') {
        ...
        this.debug(
          `Loaded ${tickets ? tickets.length : 0} tickets from all users`
        );
      } else if (this.selectedUserId === 'me') {
        // Get current user's tickets
        tickets = await zammadApi.getAssignedTickets();
        this.debug(
          `Loaded ${tickets ? tickets.length : 0} tickets assigned to current user`
        );
      } else {
        // Get tickets for specific user
        tickets = await zammadApi.getAllTickets(this.selectedUserId);
        this.debug(
          `Loaded ${tickets ? tickets.length : 0} tickets for user ${this.selectedUserId}`
        );
      }
      ...
      if (
        ...
        this.ticketsInfo.textContent =
          t('error_loading_tickets') + ': ' + error.message;
        this.ticketsInfo.className = 'info error';
      }
  ...
  async populateUserFilterFromTickets() {
    ...
    const ownerIds = [
      ...new Set(
        this.assignedTickets.map((t) => t.owner_id).filter((id) => id)
      ),
    ];
    ...
    const optionsToRemove = Array.from(this.popupUserFilter.options).filter(
      (opt) => opt.value !== 'all' && opt.value !== 'me'
    );
    ...
    if (
      currentSelection &&
      Array.from(this.popupUserFilter.options).some(
  ...
  async loadTimeHistory() {
    ...
    try {
      ...
      this.debug(
        'Received ' + (history ? history.length : 0) + ' time entries from API'
      );
      ...
      if (this.timeHistory.length === 0) {
        ...
        this.historyInfo.textContent = t('history_loaded', [
          this.timeHistory.length,
        ]);
      ...
      if (
        ...
        this.historyInfo.textContent =
          t('error_loading_history') + ': ' + error.message;
        this.historyInfo.className = 'info error';
      }
  ...
  displayAssignedTickets() {
    ...
    this.debug(
      `Filtered ${this.assignedTickets.length} tickets to ${nonClosedTickets.length} non-closed tickets`
    );
    ...
    nonClosedTickets.forEach((ticket) => {
      ...
      if (stateStr.includes('new') || stateStr.includes('open')) {
        ...
        stateStr.includes('in progress') ||
        stateStr.includes('pending')
      ) {
  ...
  async confirmDeleteTimeEntry(entryId, timeUnit, ticketId) {
    this.debug(
      `Confirming deletion of time entry ${entryId} (${timeUnit} min)`
    );

    // Show confirmation dialog
    const confirmMessage = t('confirm_delete_entry', [
      Math.round(timeUnit),
      ticketId ? `#${ticketId}` : '',
    ]);
    ...
    try {
      ...
      if (
        ...
        errorMessage +=
          "You don't have permission to delete time entries. Contact your administrator.";
      } else if (
  ...
  async loadState() {
    ...
    try {
      const result = await storage.loadMultiple([
        'zammadTrackingState',
        'zammadSettings',
        'zammadApiSettings',
      ]);
      ...
      if (state && state.isTracking && state.startTime) {
        ...
        if (lastTicketId && zammadApi.isInitialized()) {
          this.debug(
            'Refreshing info for last tracked ticket: ' + lastTicketId
          );
        ...
        if (!zammadApi.isInitialized()) {
          ...
          if (baseUrl) {
            ...
            if (settings.token && !settings.baseUrl) {
              ...
              this.debug(
                'API initialized with extracted base URL and saved token'
              );
            } else if (!settings.token) {
              this.debug(
                'No API token found - please configure in options page'
              );
  ...
  async loadTicketInfoFromApi(ticketId) {
    try {
      ...
      try {
        ...
        this.debug(
          'Ticket data received from API: ' + JSON.stringify(ticketData)
        );
        ...
        if (ticketData) {
          ...
          try {
            ...
            this.debug(
              'Time entries received from API: ' + JSON.stringify(timeEntries)
            );
        ...
        this.infoText.textContent =
          t('ticket_load_error') + ': ' + ticketError.message;
        this.infoText.className = 'info error';
  ...
  async loadTicketInfo(tab) {
    try {
      ...
      if (response && response.ticketId) {
        this.debug(
          'Ticket ID received from content script: ' + response.ticketId
        );
        ...
        if (zammadApi.isInitialized()) {
          const apiSuccess = await this.loadTicketInfoFromApi(
            response.ticketId
          );
  ...
  async startTracking() {
    try {
      ...
      try {
        ...
        this.debug(
          'Error starting tracking in content script: ' + error.message
        );
      }
  ...
  async getTicketInfo(tab) {
    ...
    try {
      ...
      if (response && response.ticketId) {
        this.debug(
          'Ticket info from content script: ' + JSON.stringify(response)
        );
    ...
    const urlPatterns = [/\/ticket.*?\/(\d+)/, /ticket.*?(\d+)/, /#.*?(\d+)/];
    ...
    try {
      ...
      try {
        ...
        if (!stopResponse || !stopResponse.success) {
          this.debug(
            'Content script could not stop tracking or time submission failed'
          );
          timeSubmissionSuccess = false;
        } else {
          this.debug(
            'Content script has stopped tracking and submitted time successfully'
          );
          timeSubmissionSuccess = true;
        }
      } catch (error) {
        this.debug(
          'Error stopping tracking in content script: ' + error.message
        );
        timeSubmissionSuccess = false;
      }
      ...
      if (timeSubmissionSuccess) {
        ...
        this.infoText.className = 'info success';
        this.debug(
          'Time tracking successfully ended with time entry submitted'
        );
      } else {
        this.infoText.textContent =
          t('time_recorded') +
        ...
        this.infoText.className = 'info warning';
        this.debug(
          'Time tracking ended but time entry not submitted - manual entry required'
        );
      }
    ...
    try {
      ...
      if (zammadApi.isInitialized() && this.currentTicketId) {
        ...
        try {
          ...
          const timeEntries = await zammadApi.getTimeEntries(
          ...
          if (Math.abs(adjustmentValue) > epsilon) {
            ...
            this.debug(
              'Submitting time adjustment: ' +
                adjustmentValue +
                ' min for date: ' +
                selectedDate
            );
            const response = await zammadApi.submitTimeEntry(
              this.currentTicketId,
              adjustmentValue,
              comment,
              selectedDate
          ...
          this.infoText.textContent =
            t('time_update_error') + ': ' + apiError.message;
          this.infoText.className = 'info error';
        }
      } else {
